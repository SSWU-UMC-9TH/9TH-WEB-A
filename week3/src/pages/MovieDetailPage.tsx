...
const MovieDetailPage = () => {
  ...
  if (isError) return <div className="text-red-500 text-2xl">에러가 발생했습니다</div>;
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-20 flex flex-col md:flex-row gap-10">
        ...
        <div className="text-white flex flex-col justify-center">
          ...
          <p className="text-yellow-400 text-lg mb-2">⭐ {movie.vote_average.toFixed(1)} / 10</p>
          ...
          <div className="flex gap-2 mb-4 flex-wrap">
            {movie.genres.map((g) => (
              <span key={g.id} className="bg-white/20 px-3 py-1 rounded-full text-sm">
                {g.name}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-6">
          {movie.credits.cast.slice(0, 12).map((person) => (
            <div key={person.id} className="flex flex-col items-center">
              ...
              <p className="text-white text-sm text-center leading-tight">{person.name}</p>
              <p className="text-gray-400 text-xs text-center">{person.character}</p>
            </div>
          ))}
        </div>
      </div>
